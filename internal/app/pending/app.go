...
var (
	...
	checkPendingBids = time.Second * 10 // Проверка ожидающих заявок
...
func (a *App) Run(ctx context.Context) error {
	...
	workers.Timeout(ctx, checkPendingBids, a.process)
...
func (a *App) process(ctx context.Context) error {
	...
	log.Info().
		Str("checkPendingBids", checkPendingBids.String()).
		Msg("checking pending bids")

	bids, err := a.repo.GetListByStatus(ctx, repository.StatusPending)
	if err != nil {
